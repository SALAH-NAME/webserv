...
class CgiHandler{// a class made to abstract working with the cgi
	...
	public:
		CgiHandler();
		void RunCgi(Request &current_req, ServerConfig &conf, LocationConfig &cgi_conf);//spawns a child and set its environment based on the request data and execve the script it will use output_pipe[0] as its stdout
