...
std::string test20()
{
	...
	{
		write(testObj.GetCgiInPipe().getWriteFd(), "line1\nline2\nline3\nline4\n", 25);
		close(testObj.GetCgiInPipe().getWriteFd());
		wait(NULL);
		...
		while (n > 0 && !testObj.ReachedCgiBodyPhase()){//passing header buffers to cgi
			...
			try {testObj.AppendCgiOutput(buffer);}
				...
				ss << "Error: an exception is throw internally (cgi output parser)\n";
				ss << ex.what() << '\n';
				return ss.str();
			}
		}
